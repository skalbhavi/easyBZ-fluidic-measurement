...
class Graph(ttk.Frame):
    def __init__(self, parent, root):
        ...
        #Set X-Axis and Y-Axis lable only for Graph 1. Later this can be made configurable 
        self.root.ax[str(1)].set_title("Pressure Generated By Eazy-Beezy")
        self.root.ax[str(1)].set_xlabel("Number of pressure readings")
        self.root.ax[str(1)].set_ylabel("Pressure in mmHg")
    ...
    def updateGraph(self, frameNum, *args, **kwargs):
        ...
        while int(serialqueue(self.root.ser)) >= int(self.root.variables['maxlength']):
            ...
            if len(val_list) != int(self.root.variables['datalength']):
                val_list = []
            #print(self.root.dataMultOff[0][0])
            #print(self.root.dataMultOff[0][1])			
            ...
            for variable in range(len(val_list)):
                try:
                    ...
                    multiplier = 0.00008987956
                    offset = -42800
                    newtmp = (tmp + offset)*multiplier
                    val_list[variable] = newtmp
                except:
                    val_list = []
                    break
            ...
            for value in val_list:
                if value != val_list[len(val_list)-1]:
                    ...
                    val_display += ' '*(val_display_width - disp_len) + \
                        '{:.1f}'.format(round(value, 1))                   
                    val += str(value)
            #print("The value received %f Multiplier: %f Offset: %f", val, offset,multiplier);
